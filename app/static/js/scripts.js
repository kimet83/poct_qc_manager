...
async function requirePassword(tabId) {
  ...
  if (!protectedTabs.includes(tabId)) {
    console.log(`Tab ${tabId} is not protected. Activating directly.`);
  ...
  // 이미 인증된 경우 바로 활성화
  if (passwordVerified) {
    console.log("Password already verified. Activating tab:", tabId);
    activateTab(tabElement, tabId); // 탭 활성화 및 LocalStorage 저장
    return true;
  }
  ...
  try {
    ...
    if (response.ok && result.verified) {
      passwordVerified = true; // 인증 성공
      console.log("Password verified successfully.");
      activateTab(tabElement, tabId); // 탭 활성화
      return true;
    } else {
...
function activateTab(tabElement, tabId) {
  ...
  if (protectedTabs.includes(tabId) && !passwordVerified) {
    console.warn("Protected tab requires password verification:", tabId);
    switchToQcTab();
    return;
  }
...
async function handleTabClick(event) {
  ...
  // 보호된 탭인지 확인
  if (protectedTabs.includes(tabId)) {
    console.log(`Tab ${tabId} is protected. Checking password.`);
    ...
    if (!isPasswordValid) {
      console.warn("Password verification failed. Switching to QC tab.");
      switchToQcTab();
      return;
    }
  }
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (savedTabId && protectedTabs.includes(savedTabId)) {
    console.warn("Protected tab detected on page load. Redirecting to QC tab.");
    switchToQcTab();
  } else if (savedTabId && savedTabElement) {
    console.log("Restoring active tab:", savedTabId);
    activateTab(savedTabElement, savedTabId);
  } else {
    switchToQcTab();
  }
  ...
  // 탭 클릭 이벤트 리스너 등록
  document
    .querySelectorAll(".nav-tabs .nav-link, .nav-pills .nav-link")
    .forEach((tab) => {
  ...
  setupExQcPlaceCodeChangeListener();
  loadQcReagent();
  loadQcResults();
...
function loadReplaceSerialsForQc(placeCode) {
  ...
  fetch(`/GetActiveSerials?placeCode=${placeCode}`)
    ...
    .then((data) => {
      deviceSerialSelect.innerHTML =
        '<option value="" disabled selected>장비를 선택하세요</option>'; // DeviceSerial 기본 옵션 추가
...
window.addEventListener("message", function (event) {
  ...
  if (event.data && event.data.type && event.data.lot) {
    if (event.data.type === "registerSerial") {
      ...
      const optionExists = Array.from(stickLotSelect.options).some(
        (option) => option.value === scannedLot
      );
...
function registerDevice() {
  const data = {
    ...
    ReplaceSerial:
      document.getElementById("registerReplaceSerial").value || null, // 공란 허용
    ReplaceReason:
      document.getElementById("registerReplaceReason").value || null, // 교체 사유 추가
  ...
  fetch("/GetSticks")
    ...
    .then((data) => {
      ...
      <tr>
        ...
        <td>
          <button class="btn btn-primary btn-sm" onclick="editStick('${
            stick.StickLot
          }')">수정</button>
          <button class="btn btn-danger btn-sm" onclick="deleteStick('${
            stick.StickLot
          }')">삭제</button>
        </td>
      </tr>
...
function updateStickInfo(stick) {
  ...
  if (stick) {
    ...
    stickLow.textContent = `${stick.LowMin || "N/A"} - ${
      stick.LowMax || "N/A"
    }`;
    stickHigh.textContent = `${stick.HighMin || "N/A"} - ${
      stick.HighMax || "N/A"
    }`;
  } else {
...
`;
  ...
  [lowResultInput, highResultInput].forEach((input) => {
    input.addEventListener("input", () => {
      ...
      const stickLowMin =
        parseFloat(
          document.getElementById("stickLow").textContent.split(" - ")[0]
        ) || null;
      const stickLowMax =
        parseFloat(
          document.getElementById("stickLow").textContent.split(" - ")[1]
        ) || null;
      const stickHighMin =
        parseFloat(
          document.getElementById("stickHigh").textContent.split(" - ")[0]
        ) || null;
      const stickHighMax =
        parseFloat(
          document.getElementById("stickHigh").textContent.split(" - ")[1]
        ) || null;
      ...
      const isLowValid =
        stickLowMin !== null &&
        stickLowMax !== null &&
        lowResult >= stickLowMin &&
        lowResult <= stickLowMax;
      const isHighValid =
        stickHighMin !== null &&
        stickHighMax !== null &&
        highResult >= stickHighMin &&
        highResult <= stickHighMax;
...
document
  .getElementById("qcDateSearch")
  .addEventListener("change", handleQcDateChange);
...
function renderResultsTable(results) {
  ...
  <tr>
    ...
    <td class="hide-on-mobile">
      ${
        result.SignImg
          ? `<img src="data:image/png;base64,${result.SignImg}" alt="서명" style="width: 50px; height: auto;">`
          : "N/A"
      }
    </td>
...
function editResult(resultId) {
  fetch(`/GetResultDetail/${resultId}`)
    ...
    .then((result) => {
      ...
      document.getElementById("editResultCheck").value =
        result.ResultCheck || "";
      document.getElementById("editDeviceCheck").value =
        result.DeviceCheck || "정상";
...
document
  .getElementById("deleteUnusedSignsButton")
    ...
    try {
      ...
      if (response.ok) {
        ...
        statusElement.textContent = `Error: ${
          data.error || "Failed to delete unused signs"
        }`;
      }
    } catch (error) {
...
function generateReport() {
  ...
  fetch("/generateReport", {
    ...
    .then((data) => {
      ...
      const reportManagerDisplay = document.getElementById(
        "reportManagerDisplay"
      );
  ...
  `;
      ...
      <tr>
        ...
        <td rowspan="2">${
          row.SignImg
            ? `<img src="data:image/png;base64,${row.SignImg}" style="width:50px;">`
            : ""
        }</td>            
        ...
        data.forEach((place) => {
          const row = `
                      <tr>
                          ...
                          <td>
                              <button class="btn btn-primary btn-sm" onclick="editPlace(${
                                place.PlaceId
                              })">수정</button>
                              <button class="btn btn-danger btn-sm" onclick="deletePlace(${
                                place.PlaceId
                              })">삭제</button>
                          </td>
                      </tr>
